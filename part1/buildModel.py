...
if __name__ == '__main__':
    ...
    for eachword in spamWordCount:
        file.write(eachword + '\t' + '{0:.16f}'.format((spamWordCount[eachword]/ (sum(spamWordCount.values()) * 1.0)) * numOfSpams) + '\t' + '{0:.16f}'.format(( 1 / (len(spamWordCount) * 1.0)) * numOfSpams) + '\t' + str(spamWordCount[eachword]) + '\t' + 's')
        file.write(os.linesep)
    for eachword in nonspamWordCount:
        file.write(eachword + '\t' + '{0:.16f}'.format((nonspamWordCount[eachword] / (sum(nonspamWordCount.values()) * 1.0)) * numOfNonSpams) + '\t' + '{0:.16f}'.format(( 1 / (len(nonspamWordCount) * 1.0)) * numOfSpams) + '\t' + str(nonspamWordCount[eachword]) + '\t'+ 'ns')
        file.write(os.linesep)
    file.close()
